"""
LLM-as-a-Judge: score baseline / silver / gnnexp / pgexp text explanations
produced by GNNExplainer_get_explanation_from_edge_mask.py.

Rubric (1-5 each):
  - Plausibility  : likelihood that the explanation is correct per domain knowledge
...
For every idx the judge sees:
  case text  + article text  (grounding context)
...
----------------------------------------
The **generator** (SaulLM) and the **judge** are deliberately kept as *separate
models* loaded independently.  Using the same model to score its own outputs
introduces Self-Enhancement Bias — the model systematically over-scores text
that matches its own generation style.
...
Three judge tiers are supported; select via JUDGE_TIER below:
  ...
  Option A — Selene 1 Mini  (JUDGE_TIER = "selene")   ← RECOMMENDED
    ...
    to the same extreme (the "only 1 or 5" problem), because it can reason
    freely about one criterion at a time without the constraint of fitting
    multiple scores into a single JSON token budget.
  ...
  Option B — Specialized Evaluator  (JUDGE_TIER = "prometheus")
    ...
    prefer Selene when calibrated mid-range scores are needed.
...
from Graph_utils import getNodeText  # noqa: E402

explainer = "pg"  # gnn | pg | pagelink

category     = "tp"
EXPL_ROOT    = os.path.join(PROJECT_ROOT, "legal_exp", "results", "explanations",
                             f"hetero_{explainer}_explainer")
...
dataset = load_dataset("ecthr_cases", "violation-prediction")["train"].to_pandas()
...
class JudgeConfig:
    ...
    # Selene needs room for chain-of-thought before the score.
    max_new_tokens: int
...
judge_tokenizer = AutoTokenizer.from_pretrained(judge_cfg.model_id, use_fast=True)
...
def _generate_judge(messages: list[dict], do_sample: bool = False) -> str:
    ...
    coherent but the score is still calibrated (greedy can get anchored).
...
# to produce multiple constrained values in a single response.

_SELENE_SYSTEM = (
    "You are a fair and objective evaluator assessing the quality of "
    "AI-generated legal arguments. "
    "You will be given an instruction (a legal case and an article allegedly "
    "violated) and a response (the argument to evaluate). "
...
_SELENE_RUBRICS: dict[str, str] = {
    "plausibility": (
        "Plausibility — factual correctness and legal coherence.\n"
        "1 = Factually wrong or legally incoherent.\n"
        "2 = Mostly incorrect; only minor correct elements.\n"
        "3 = Partially correct; some notable errors.\n"
        "4 = Mostly correct with only minor gaps.\n"
        "5 = Factually accurate and legally sound throughout."
    ),
    "completeness": (
        "Completeness — self-sufficiency; all context needed is present.\n"
...
def _build_selene_messages(
    case_txt: str, article_txt: str, explanation: str, criterion: str
) -> list[dict]:
    """Return a message list to score one criterion with Selene."""
    budget = MAX_CONTEXT_TOKENS // 3
    case_txt    = _truncate_to_tokens(case_txt,    budget)
    article_txt = _truncate_to_tokens(article_txt, budget)
    explanation = _truncate_to_tokens(explanation, budget)

    user_content = (
        "[Instruction]\n"
        f"## Case\n{case_txt.strip()}\n\n"
        f"## Article allegedly violated\n{article_txt.strip()}\n\n"
        "[Response]\n"
        f"{explanation.strip()}\n\n"
        "[Scoring Rubric]\n"
...
# ── Flow-Judge ───────────────────────────────────────────────────────────────
# Flow-Judge uses Phi-3.5-mini's chat template (supports system role).
# Output format: <feedback>...</feedback>\n<score>N</score>
# We score each criterion independently — one call per criterion —
# to get calibrated scores across the full 1-5 Likert scale.

_FLOW_RUBRICS_LEGAL: dict[str, tuple[str, str]] = {
    # (evaluation_criteria, scoring_rubric)
    "plausibility": (
        "Plausibility of the legal argument on a 5-point Likert scale: "
        "how factually correct and legally coherent is the explanation?",
        "Score 1: The argument is factually wrong or legally incoherent.\n"
        "Score 2: The argument is mostly incorrect with only minor correct elements.\n"
        "Score 3: The argument is partially correct but contains notable legal errors.\n"
        "Score 4: The argument is mostly correct with only minor gaps or imprecisions.\n"
        "Score 5: The argument is factually accurate and legally sound throughout.",
    ),
    "completeness": (
        "Completeness of the legal argument on a 5-point Likert scale: "
    ...
    "compactness": (
        "Compactness of the legal argument on a 5-point Likert scale: "
...
def _build_flow_messages_legal(
    case_txt: str, article_txt: str, explanation: str, criterion: str
) -> list[dict]:
    """Build a Flow-Judge message list for one legal criterion."""
    budget = MAX_CONTEXT_TOKENS // 3
    case_txt    = _truncate_to_tokens(case_txt,    budget)
    article_txt = _truncate_to_tokens(article_txt, budget)
    explanation = _truncate_to_tokens(explanation, budget)

    eval_criteria, rubric = _FLOW_RUBRICS_LEGAL[criterion]
    ...
    user_content = (
        ...
        "<inputs>\n"
        f"<case>\n{case_txt.strip()}\n</case>\n"
        f"<article_allegedly_violated>\n{article_txt.strip()}\n</article_allegedly_violated>\n"
        "</inputs>\n\n"
        "# OUTPUT\n"
...
_RUBRIC_PROMETHEUS = """\
[System]
You are a fair and objective evaluator that assesses the quality of AI-generated legal explanations.

[Task Description]
An instruction (comprising a legal case and an article allegedly violated) will be given along with a response (the explanation to score).
Your task is to evaluate the response by scoring it on exactly three dimensions and returning a JSON object.

<rubric>
  plausibility  (1-5): factual correctness and legal coherence
...
def _build_messages_prometheus(
    case_txt: str, article_txt: str, explanation: str
) -> list[dict]:
    instruction = (
        "## Case\n" + case_txt.strip() + "\n\n"
        "## Article\n" + article_txt.strip()
...
_RUBRIC_JSON = (
    "Score the following legal explanation on exactly three criteria "
    "(integer 1-5 each):\n"
    "  plausibility  – factual correctness and legal coherence\n"
    "  completeness  – self-sufficiency; all context needed is present\n"
...
def _build_messages_general_judge(
    ...
    user_content = (
        "## Case\n" + _truncate_to_tokens(case_txt, budget_each).strip() + "\n\n"
        "## Article\n" + _truncate_to_tokens(article_txt, budget_each).strip() + "\n\n"
        "## Explanation\n" + _truncate_to_tokens(explanation, budget_each).strip()
    )
    return _build_messages_general(
        system_content="You are an expert legal AI evaluator.\n\n" + _RUBRIC_JSON,
...
def _extract_scores_json(raw: str) -> dict | None:
    ...
    # Strategy 2 & 3 — first / all brace blobs
    ...
    # Strategy 4 — key-value regex scan
    found: dict[str, int] = {}
    ...
    # Strategy 5 — positional named-group regex
    m = re.compile(
...
def _score_with_selene(
    explanation: str, case_txt: str, article_txt: str, max_retries: int
) -> dict | None:
    """
    ...
    for criterion in ("plausibility", "completeness", "compactness"):
        ...
        for attempt in range(1, max_retries + 1):
            messages = _build_selene_messages(case_txt, article_txt, explanation, criterion)
...
def _score_with_flow(
    ...
    for criterion in ("plausibility", "completeness", "compactness"):
        score = None
        for attempt in range(1, max_retries + 1):
            messages = _build_flow_messages_legal(case_txt, article_txt, explanation, criterion)
...
def score_file(
    ...
    if judge_cfg.output_format == "selene":
        return _score_with_selene(explanation, case_txt, article_txt, max_retries)

    if judge_cfg.output_format == "flow":
        return _score_with_flow(explanation, case_txt, article_txt, max_retries)
    ...
    for attempt in range(1, max_retries + 1):
        if attempt == 1:
            if judge_cfg.output_format == "prometheus":
                messages = _build_messages_prometheus(case_txt, article_txt, explanation)
            else:
                messages = _build_messages_general_judge(case_txt, article_txt, explanation)
        else:
            # Repair prompt: show the model its broken output
            repair_suffix = (
...
# ---------------------------------------------------------------------------
# Index discovery & main
# ---------------------------------------------------------------------------

def discover_indices(text_dir: str) -> list[int]:
    indices = set()
    for fname in os.listdir(text_dir):
        m = re.match(r"(?:baseline|silver|gnnexp|pgexp|plink)(\d+)\.txt$", fname)
        if m:
...
def main() -> None:
    ...
    if explainer == "gnn":
        VARIANTS = ["baseline", "silver", "gnnexp"]
    elif explainer == "pg":
        VARIANTS = ["pgexp"]
    elif explainer == "pagelink":
        VARIANTS = ["plink"]
    else:
        VARIANTS = []
    ...
    try:
        for idx in tqdm(indices, desc="Indices"):
            ...
            try:
                em_pkl = os.path.join(
                    CATEGORY_DIR,
                    f"{explainer}exp_gnn_hier_graph_3GATConv_pred_edge_to_comp_g_edge_mask.pkl",
                )
                edge_masks_raw = pickle.load(open(em_pkl, "rb"))

                from Graph_utils import fact_case_offset
                ...
                for nodes, _ in edge_masks_raw.items():
                    ...
                    if counter > 100:
                        break
                    if counter in links_to_skip:
                        continue
                    if counter != idx:
                        continue

                    src_ntype = nodes[0][0]
                    src_nid   = nodes[0][1] if explainer == "pg" else nodes[0][1].item()
                    tgt_ntype = nodes[1][0]
                    tgt_nid   = nodes[1][1] if explainer == "pg" else nodes[1][1].item()
                ...
                print(f"  [WARN] Could not load case/article text for idx={idx}: {exc}")
                case_txt    = "[Case text unavailable]"
                article_txt = "[Article text unavailable]"
            ...
            for variant in VARIANTS:
                ...
                scores = score_file(fpath, case_txt, article_txt)
